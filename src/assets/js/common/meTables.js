    ...
    meTables.fn = meTables.prototype = {
        ...
        init: function (params) {
            ...
            if (this.options.bChildTables) {
                ...
                // 新增、查看、编辑、删除
                $('.me-table-child-create').click(function (evt) {
                    evt.preventDefault();
                    self.create(true);
                });
                $('.me-table-child-save').click(function (evt) {
                    evt.preventDefault();
                    self.save(null, true);
                });
                $(document).on('click', '.me-table-child-detail', function (evt) {
                    evt.preventDefault();
                    self.detail($(this).attr('table-data'), true)
                });
                $(document).on('click', '.me-table-child-update', function (evt) {
                    evt.preventDefault();
                    self.update($(this).attr('table-data'), true)
                });
                $(document).on('click', '.me-table-child-delete', function (evt) {
                    evt.preventDefault();
                    self.delete($(this).attr('table-data'), true)
                });
                ...
                $(this.options.sTable + ' tbody').on('click', this.options.childTables.sClickSelect, function () {
                    ...
                    tr.siblings(tr).each(function () {
                        if (self.table.row($(this)).child.isShown()) {
                            self.table.row($(this)).child.hide();
